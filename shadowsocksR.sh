...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/Aiyuanjiang/jinsama/jin/bbr.sh; then
...
Set_config_port(){
	...
	read -e -p "(默认: 52147):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="52147"
...
Set_config_password(){
	...
	read -e -p "(默认: xiongjin147):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="xiongjin147"
